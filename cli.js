...
const ATTEST_KEYS = {
  0xa0: {
    ...
    publicKey:
      '0436de4602f2d85d9cbd4deac220173b4e9f32aae7e808278aa34d6c6a88482b4353b22e017b02427a5f8b71d042c0a444886169ef09371652a33504c834f0aff9',
  },
};
...
async function readPublicKeys(execCmd) {
  ...
  const brand = 'hellstar';
  const supplykey = [brand].join('/');
  var data = JSON.parse(
    await fs.promises.readFile(__dirname + '/chips.json', 'utf-8')
  );
  if (pkey1 in data) {
    console.error('ALREADY SCANNED!');
    setTimeout(function () {
      debugger;
    }, 5000);
  } else {
    ...
    if (supplykey in data) {
      data[supplykey] += 1;
    } else {
      data[supplykey] = 1;
    }
  }
  const savedData = JSON.stringify(data, null, 4);
  const toSave = await fs.promises.writeFile(
...
if (args[0] === 'sign' || args[0] === 'sign_raw') {
  ...
  cb = async (execCmd) => {
    ...
    for (let i = 0; i < 2; i++) {
      if (
        pkeys['pkey' + keyNo] ===
        ec.recoverPubKey(hexToDecimal(challengeHash), fixedSig, i).encode('hex')
      ) {
        recoveryParam = i;
        break;
      }
    ...
    if (pkeyAddress !== recoveredAddress) {
      return {
        error: 'Failed to correctly recover public key from the signature.',
      };
    }
    ...
    if (res[0] === 0xe1 && res.length === 2) {
      if (res[1] === 0x08) {
        ...
        if (existingData.compare(latchData) === 0) {
          return {
            success: true,
            message: 'Same data is already stored in this latch.',
          };
        } else {
          return {
            success: false,
            message: 'Different data is already stored in this latch.',
          };
        }
      } else {
        return {
          success: false,
          message: 'Latch failed with error code: ' + res.toString('hex'),
        };
      }
    }
  ...
  cb = async (execCmd, readNdef) => {
    ...
    return {
      ...
      _:
        'We have verified that the issuer signature is correct and the tag is able to sign data using these keys.',
    };
  };
    ...
    if (res[0] === 0xe1) {
      if (res[1] === 0x06) {
        return {
          success: true,
          executed: false,
          message: 'Third key was already generated.',
        };
      } else {
        return {
          success: false,
          executed: false,
          message: 'Failed to generate key. Error code: ' + res.toString('hex'),
        };
      }
    }
  ...
  console.error(
    '    You must manually calculate hash of the data you want to sign.'
  );
  console.error(
    '    Any 32 byte long hash is accepted, especially the output of'
  );
  console.error(
    "    Etherum's Keccak-256(.), or classic SHA-256(.), or BLAKE-256(.)"
  );
  console.error('');
  ...
  console.error(
    '        sign 1 793d29ac53e8f8eda22fb298bf5d3df646aa8ca6dae5de3e20c086a4aa160827'
  );
  console.error('');
  console.error('');
  console.error('    sign <key_no> <data>');
  console.error(
    '    Perform ECDSA(Keccak-256(data)) signature using NFC card.'
  );
  console.error(
    "    The provided data is automatically hashed using Ethereum's"
  );
  ...
  console.error(
    '    This command will also fetch and validate the attestation'
  );
  ...
  console.error(
    '    and whether it was really signed by the currently tapped NFC tag.'
  );
  console.error('');
  console.error('    Parameters:');
  ...
  console.error('    Example invocation:');
  console.error(
    '        latch 1 e5d961f8dad5f030f57e88e72aad26492d722afb5cc84256dca2ba6362bebf51'
  );
  console.error('');
  process.exit(1);
}
...
function stopPCSC(code) {
  ...
  if (code !== 'done') {
    player.play('disconnect.wav', function (err) {
      if (err) throw err;
    });
...
async function readNDEF(reader) {
  let resSelect = await reader.transmit(
    Buffer.from('00A4040007D276000085010100', 'hex'),
    255
  );
  ...
  let resSelectFile = await reader.transmit(
    Buffer.from('00A4000C02E10400', 'hex'),
    255
  );
...
async function executeCommand(reader, payload) {
  let resSelect = await reader.transmit(
    Buffer.from('00A4040007481199130e9f0100', 'hex'),
    255
  );
...
nfc.on('reader', (reader) => {
  ...
  reader.on('card', async (card) => {
    clearTimeout(timeout);
    timeout = setTimeout(stopPCSC, 90000, 'timeout');
    let res = null;

    try {
      res = await cb(
        (payload) => executeCommand(reader, payload),
        () => readNDEF(reader)
      );
    } catch (e) {
      console.error(e);
